...
import prisma from 'app/lib/prisma'; // Importa tu instancia de PrismaClient
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (typeof duration !== 'number' || duration <= 0) {
      ...
      return NextResponse.json({ message: 'Duración total de sesión inválida.' }, { status: 400 });
    }
    if (typeof cycles_completed !== 'number' || cycles_completed < 0) { // Puede ser 0 si no se completó un ciclo completo
      console.warn('POST /api/pomodoro: Ciclos completados inválidos.');
      return NextResponse.json({ message: 'Ciclos completados inválidos.' }, { status: 400 });
    }
    if (typeof work_duration_seconds !== 'number' || work_duration_seconds < 0) {
      console.warn('POST /api/pomodoro: Duración de trabajo inválida.');
      return NextResponse.json({ message: 'Duración de trabajo inválida.' }, { status: 400 });
    }
    if (typeof break_duration_seconds !== 'number' || break_duration_seconds < 0) {
      console.warn('POST /api/pomodoro: Duración de descanso inválida.');
      return NextResponse.json({ message: 'Duración de descanso inválida.' }, { status: 400 });
    }
    ...
    console.log('POST /api/pomodoro: Sesión Pomodoro guardada con éxito:', newPomodoroSession.id);
    ...
    console.error('POST /api/pomodoro: Error al guardar la sesión Pomodoro:', error);

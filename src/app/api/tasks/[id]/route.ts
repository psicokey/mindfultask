...
export async function GET({ params }: { params: { id: string } }) {
  ...
  if (isNaN(taskId)) {
    console.error(
      `GET /api/tasks/${params.id}: ID de tarea inválido: ${params.id}`
    );
  ...
  try {
    ...
    if (task.userId !== userId) {
      console.warn(
        `GET /api/tasks/${params.id}: Acceso denegado. El usuario ${userId} intentó acceder a la tarea de otro usuario (${task.userId}).`
      );
      return NextResponse.json(
    ...
    console.log(
      `GET /api/tasks/${params.id}: Tarea encontrada con éxito:`,
      task.id
    );
    return NextResponse.json(task, { status: 200 });
  } catch (error) {
    console.error(
      `GET /api/tasks/${params.id}: Error al obtener la tarea:`,
      error
    );
...
export async function PUT(
  ...
  if (isNaN(taskId)) {
    console.error(
      `PUT /api/tasks/${params.id}: ID de tarea inválido: ${params.id}`
    );
    return NextResponse.json(
      { message: "ID de tarea inválido." },
      { status: 400 }
  ...
  try {
    ...
    if (existingTask.userId !== userId) {
      console.warn(
        `PUT /api/tasks/${params.id}: Acceso denegado. El usuario ${userId} intentó actualizar la tarea de otro usuario (${existingTask.userId}).`
      );
      return NextResponse.json(
    ...
    console.log(
      `PUT /api/tasks/${params.id}: Tarea actualizada con éxito:`,
      updatedTask.id
    );
    return NextResponse.json(
      {
...
export async function DELETE(
  ...
  if (isNaN(taskId)) {
    console.error(
      `DELETE /api/tasks/${params.id}: ID de tarea inválido: ${params.id}`
    );
    return NextResponse.json(
      { message: "ID de tarea inválido." },
      { status: 400 }
  ...
  try {
    ...
    if (existingTask.userId !== userId) {
      console.warn(
        `DELETE /api/tasks/${params.id}: Acceso denegado. El usuario ${userId} intentó eliminar la tarea de otro usuario (${existingTask.userId}).`
      );
      return NextResponse.json(
    ...
    console.log(
      `DELETE /api/tasks/${params.id}: Tarea eliminada con éxito:`,
      taskId
    );
    return NextResponse.json(
